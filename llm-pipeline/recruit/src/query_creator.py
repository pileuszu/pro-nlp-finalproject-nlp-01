...
class QueryCreator:
    """
    사용자의 포트폴리오 데이터를 분석하여 google-genai SDK를 통해 
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        제공된 사용자의 정보를 바탕으로, 검색 엔진(Vector DB)에서 가장 적합한 공고를 찾기 위한 3가지 검색 쿼리를 생성해주세요.
        출력은 반드시 한국어로 작성하며, 검색 엔진이 의미적으로 잘 찾아낼 수 있도록 핵심 키워드와 맥락을 포함해야 합니다.

        필수 생성 쿼리 유형:
        1. query_a (기술 스택 중심): 사용자의 언어, 프레임워크, 기술 도구와 핵심 역량 매칭
        2. query_b (문제 해결 중심): 사용자의 프로젝트 성과, TPS, 최적화 경험 등 구체적 성과 매칭
        3. query_c (프로젝트 요약): 전체적인 도메인 경험 및 주요 업무 경험 요약 매칭
        
        응답은 반드시 'query_a', 'query_b', 'query_c'를 키로 하는 JSON 형식이어야 합니다.
        """

    def generate_queries(self, user_data: Dict) -> Dict[str, str]:
        """
        입력받은 user_data를 바탕으로 JSON 모드를 사용하여 3가지 검색용 쿼리를 생성합니다.
        """
        
        user_input = f"다음 사용자 데이터를 분석해서 검색용 쿼리 3개를 생성해줘:\n{json.dumps(user_data, ensure_ascii=False)}"

        try:
            # google-genai SDK 형식 호출
            ...
            # JSON 모드 사용 시 response.text는 순수 JSON 문자열임
            queries = json.loads(response.text)
            
            return {
                "query_a": queries.get("query_a", ""),
                "query_b": queries.get("query_b", ""),
                "query_c": queries.get("query_c", "")
            }
            ...
            return {
                "query_a": f"{profile.get('job_title', '')} 핵심 기술 매칭",
                "query_b": "성능 최적화 및 문제 해결 경험",
                "query_c": "백엔드 시스템 구축 프로젝트 요약"
            }
    ...
    def rank_final_recommendations(self, user_data: Dict, candidates: List[Any]) -> Dict:
        ...
        try:
            response = self.client.models.generate_content(
                ...
                contents=[prompt],
                config=types.GenerateContentConfig(
                    system_instruction="당신은 정교한 매칭을 수행하는 채용 전문가입니다.",
                    response_mime_type="application/json"
                )
            )
